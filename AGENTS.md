...
Shared invariants for scientific presentation projects based on this template. Task procedures live in the focused skills and `agent-workflows/`.

## Skill routing

- Use `make-presentation` to plan or build a new deck, make substantive slide revisions, or reconnect slides to changed analysis outputs.
- Use `review-presentation` when an approved or existing deck needs rendering, screenshots, visual correction, or delivery checks.
- In this template repository only, use `maintain-presentation-template` when changing supported patterns, the catalogue, learning hub, installer, or validation workflow.
...
- Keep visible slide text concise, professional, factual, and free of filler or AIisms.
- Do not invent text, results, or examples merely to fill space in a project deck.
- Before editing a deck, read `README.md`, `slide-plan.md`, and `presentation.yml`, then learn the surrounding analysis project.
- For a new deck, discuss audience, duration, purpose, meeting context, one-sentence story, and output filename; use `presentation.html` by default.
...
- Give every slide one claim and one dominant proof object.
...
- Never transcribe numeric results from another deck when a rerenderable source exists.
...
- Prefer the reusable patterns demonstrated in the learning hub and implemented in `assets/theme.scss`.
...
- Reserve arbitrary absolute positioning for genuine hero images or small accents; use the supported column and edge-figure patterns for scientific figures.
...
## Render and delivery invariants
...
- Check every slide for overflow, tiny labels, weak hierarchy, awkward spacing, crowded tables, and unclear proof objects.
...
- Keep generated HTML, screenshots, contact sheets, and generated background assets out of version control.

## Installation invariants

- Deck-level files are required; project-root skill shims are useful but optional and must warn rather than abort when they cannot be installed.
